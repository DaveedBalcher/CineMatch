...
struct MainView: View {
    ...
    var body: some View {
        VStack {
            ...
            case .login:
                LoginView() { name in
                    vm.createUser(with: name)
                }
            case let .quiz(movie, ratingsLeft: ratingsLeft):
                QuizView(vm: QuizViewModel(movie: movie, ratingsLeft: ratingsLeft)) { rating in
                    vm.saveRating(for: movie, rating: rating)
                }
            case .sync:
                SyncView(loggedInUser: vm.user!.name, users: vm.getAllUserNames()) { selectedUser in
                    vm.fetchRecommendations(for: selectedUser)
